...
-   **Dormitory Bed Capacity Tracking**: Simplified bed-level tracking for dormitory rooms using new `/api/rooms/:id/bed-inventory` endpoint. When a dormitory room is selected, the system fetches real-time bed availability (totalBeds, reservedBeds, remainingBeds) and displays clear status (e.g., "2 of 6 beds occupied • 4 beds available"). Users select the number of beds to book via a numeric input, preventing overbooking by limiting the max value to remaining beds.
